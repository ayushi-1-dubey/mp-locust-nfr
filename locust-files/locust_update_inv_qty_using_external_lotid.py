...
class TesterClient:
    ...
    def update_inv(self, request: distributor_service_pb2.UpdateInventoryQuantityUsingExternalLotIdRequest) -> distributor_service_pb2.UpdateInventoryQuantityUsingExternalLotIdResponse:
        ...
        resp = stub.UpdateInventoryQuantityUsingExternalLotId(request=request)
        print(resp)
...
class PerfTaskSet(TaskSet):
    ...
    def create_req_payload(self):
        with open("sample-load-data/update_inv_using_external_lot_id.json", "r" ) as jsonfile:
    ...
    def update_inv(self):
        ...
        for req in range(0, len(dict)):
            ...
            req_data = distributor_service_pb2.UpdateInventoryQuantityUsingExternalLotIdRequest(distributor_apob_id="1", lot_attributes=self.lot_attributes(payload_dict), external_lot_id=self.external_lot_id(payload_dict))
            self.locust_request_handler("update_inventory_using_external_lot_id", req_data)
    ...
    def _get_request_function(self, grpc_name):
        req_func_map = {
            "update_inventory_using_external_lot_id": self.client.update_inv
        }
...
class UpdateInventoryUsingExternalLotId(User):
    tasks = [PerfTaskSet]
